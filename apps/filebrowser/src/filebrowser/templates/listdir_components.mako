  ...
  <script>
    ...
    var FileBrowserModel = function (files, page, breadcrumbs, currentDirPath) {
      ...
      self.editFile = function () {
        huePubSub.publish('open.link', "${url('filebrowser_views_edit', path='')}" + encodeURIComponent(self.selectedFile().path));
      };

      self.downloadFile = function () {
        huePubSub.publish('open.link', "${url('filebrowser_views_download', path='')}" + encodeURIComponent(self.selectedFile().path));
      };
